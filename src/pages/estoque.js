...
import { cadastrarItemEstoque, listarItensEstoque, listarAlertasEstoqueBaixo, calcularCustoUnitario } from "../services/estoque-service.js";
...
function card(item) { const atual = normalizarNumero(item.quantidadeAtual); const minimo = Math.max(normalizarNumero(item.quantidadeMinima), 1); const pct = Math.min((atual / minimo) * 100, 100); return `<article class="stock-card"><strong>${escapar(item.nome)}</strong><span>${escapar(item.categoria)} · ${escapar(item.marca || "Sem marca")}</span><div class="stock-meta"><span>Atual: ${atual} ${escapar(item.unidadeMedida)}</span><span>Custo: ${formatarMoeda(calcularCustoUnitario(item))}</span></div><div class="stock-bar"><i style="width:${pct}%"></i></div></article>`; }

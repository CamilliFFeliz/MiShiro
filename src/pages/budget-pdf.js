...
async function gerar(tipo) {
  ...
  const materiais = Array.from(estadoOrcamento.carrinho.values()).map(({item,quantidade}) => ({ item, quantidade, valor:(item.precoEmbalagem / Math.max(item.quantidadeEmbalagem || 1,1)) * quantidade }));

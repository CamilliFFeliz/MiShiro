...
async function carregarEdicao() {
  ...
  itens.forEach((snapshot) => { const item = mapa.get(snapshot.itemEstoqueId) || itemLegado(snapshot); estadoOrcamento.carrinho.set(item.id, { item, quantidade: normalizarNumero(snapshot.quantidadeUsada) }); });

...
function cardEstoque(item) {
  ...
  return `<article class="ops-stock-item" data-stock-id="${escapar(item.id)}"><div class="ops-stock-item__title"><strong>${escapar(item.nome)}</strong><span class="ops-category-badge">${escapar(item.categoria)}</span></div><div class="ops-stock-item__meta"><span>${formatarMoeda(calcularCustoUnitario(item))} por ${escapar(resumoUnidade(item))}</span><span>${escapar(getItemSpecification(item) || "Sem especificação adicional")}</span></div><div class="ops-stock-item__bottom"><div class="ops-stock-item__available"><span>Disponível</span><strong>${formatarQuantidade(disponivel)} ${escapar(resumoUnidade(item))}</strong></div>${stepper(selecionada, disponivel)}</div></article>`;
}

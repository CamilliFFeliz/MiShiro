...
export function iniciarBancoLocal() { return abrirBancoLocal(); }
export function migrarBancoLocal() { return abrirBancoLocal(); }
...
export async function salvarVarios(nomeLoja, registros) {
  ...
  await executarTransacao([nomeLoja], "readwrite", async ({ lojas }) => {
    for (const registro of lista) await converterRequisicao(lojas[nomeLoja].put(registro));
  });
  return lista.length;
}
...
export async function exportarTodasAsLojas() {
  const dados = {};
  for (const nomeLoja of Object.values(LOJAS)) dados[nomeLoja] = await obterTodos(nomeLoja);
  return dados;
}
...
export async function substituirTodasAsLojas(dadosPorLoja) {
  ...
  await executarTransacao(nomesLojas, "readwrite", async ({ lojas }) => {
    for (const nomeLoja of nomesLojas) await converterRequisicao(lojas[nomeLoja].clear());
    for (const nomeLoja of nomesLojas) {
      const registros = Array.isArray(dadosPorLoja?.[nomeLoja]) ? dadosPorLoja[nomeLoja] : [];
      for (const registro of registros) await converterRequisicao(lojas[nomeLoja].put(registro));
    }
  });
  return true;
}
...
function abrirBancoLocal() {
  ...
  if (!promessaBanco) {
    promessaBanco = new Promise((resolver, rejeitar) => {
      ...
      requisicao.addEventListener("upgradeneeded", () => configurarBanco(requisicao.result, requisicao.transaction));

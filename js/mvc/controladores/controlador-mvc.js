...
function criarNavegacaoMvc() {
  ...
  const botoes = [
    ["pipelineMvc", "kanban-square", "Pipeline"],
    ["agendaMvc", "calendar-days", "Agenda"],
    ["backupMvc", "database-backup", "Backup"]
  ];

  botoes.forEach(([tela, icone, texto]) => {
...
function criarTelasMvc() {
  ...
  conteudo.insertAdjacentHTML("beforeend", `
    ...
    <section class="screen-panel tela-mvc" id="agendaMvcScreen" data-screen="agendaMvc">
      ...
      <div class="agenda-mvc" id="agendaMvc"></div>
    </section>
    <section class="screen-panel tela-mvc" id="backupMvcScreen" data-screen="backupMvc">
      ...
      <div class="backup-mvc glass-panel">
        <p>O GitHub Pages não grava um arquivo de banco no servidor. O MiShiro usa IndexedDB no navegador e backup JSON para proteção.</p>
        <div class="backup-acoes-mvc">
          <button class="primary-button" id="exportarBackupMvc" type="button"><i data-lucide="download"></i>Exportar backup completo</button>
          <button class="secondary-button" id="importarBackupMvc" type="button"><i data-lucide="upload"></i>Importar backup</button>
          <button class="ghost-button" id="persistenciaMvc" type="button"><i data-lucide="shield-check"></i>Ativar persistência local</button>
        </div>
        <input id="arquivoBackupMvc" type="file" accept="application/json,.json" hidden />
        <p class="status-mvc" id="statusBackupMvc" aria-live="polite"></p>
      </div>
    </section>
  `);
...
function vincularEventosMvc() {
  ...
  document.querySelector("#exportarBackupMvc")?.addEventListener("click", async () => {
    await exportarBackupCompleto();
    mostrarStatusBackup("Backup completo exportado.");
  });
...
async function renderizarAgendaMvc() {
  ...
  elementos.agenda.innerHTML = agendamentos.length ? agendamentos.map((item) => `<article class="card-agenda-mvc"><strong>${item.data || "Sem data"} ${item.horaInicio || ""}</strong><span>${item.status}</span><p>Orçamento: ${item.orcamentoId}</p><p>${item.observacoes || "Sem observações."}</p></article>`).join("") : "<p class='vazio-mvc'>Nenhum agendamento criado.</p>";
}
